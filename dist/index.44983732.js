const API_URL = "https://api.themoviedb.org/3/discover/movie?api_key=6a06f6cef3d768e50f7cebabd4d86561";
...
selected.addEventListener("change", (e)=>{
    ...
    if (dropdownValue == 1) moviedb = API_URL + "&sort_by=vote_count.desc&page=1";
    else if (dropdownValue == 2) moviedb = API_URL + "&sort_by=popularity.desc&page=1";
    else if (dropdownValue == 3) moviedb = API_URL + "&sort_by=vote_average.desc&page=1";
    else if (dropdownValue == 4) moviedb = API_URL + "&sort_by=original_title.asc&page=1";
    else if (dropdownValue == 5) moviedb = API_URL + "&sort_by=release_date.desc&page=1";
...
fetch(moviedb).then((response)=>response.json()).then((response)=>response.results).then((response)=>response.slice(0, 5)).then((response)=>{
